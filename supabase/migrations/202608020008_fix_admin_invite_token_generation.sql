-- Schema-qualify token generation for security-definer admin invitations.
...
as $$
declare
  raw_token text := encode(public.gen_random_bytes(24), 'hex');
...
begin
  ...
  values (p_member_id, encode(public.digest(raw_token, 'sha256'), 'hex'), auth.uid(), now() + interval '7 days')
