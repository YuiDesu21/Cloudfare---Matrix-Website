document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadEntry() {
    const { data: requests, error } = await window.matrixSupabase.rpc("admin_get_entry_requests");
    if (error) return show(alertBox, error.message, "danger");
    ...
    list.innerHTML = pending.map(request => {
      const uplineLabel = request.matrixUplineMemberId
        ? `${escapeHtml(request.matrixUplineName || "Selected member")} (@${escapeHtml(request.matrixUplineUsername || "-")})`
        : "No upline code saved";
      const slots = request.matrixUplineSlotsLeft == null ? "-" : `${Number(request.matrixUplineSlotsLeft)} open direct slot(s)`;
      return `<article class="portal-card withdrawal-history-item" data-request-id="${escapeHtml(request.id)}"><div class="withdrawal-history-topline"><div><span class="withdrawal-reference-label">${escapeHtml(request.accountCode)}</span><h2>${escapeHtml(request.fullName)}</h2></div><span class="withdrawal-status status-pending">Pending</span></div><div class="withdrawal-history-details"><div><span>Username</span><strong>@${escapeHtml(request.username)}</strong></div><div><span>Reference</span><strong>${escapeHtml(request.referenceNumber)}</strong></div><div><span>Amount</span><strong>PHP ${Number(request.amount).toLocaleString()}</strong></div><div><span>Requested</span><strong>${new Date(request.createdAt).toLocaleString()}</strong></div><div><span>1200 Upline</span><strong>${uplineLabel}</strong></div><div><span>Upline code</span><strong>${escapeHtml(request.matrixUplineCode || "-")}</strong></div><div><span>Upline slots</span><strong>${escapeHtml(slots)}</strong></div></div><p class="withdrawal-history-note">Approval places this member directly under the selected 1200 Matrix upline. If the upline reaches 3 direct downlines before approval, this approval will be blocked.</p><div class="balance-card-buttons"><button class="button button-primary button-small approve-entry" type="button">Approve &amp; Place</button><button class="button button-outline button-small reject-entry" type="button">Reject</button></div></article>`;
    }).join("");
    ...
    list.querySelectorAll("[data-request-id]").forEach(card => {
      card.querySelector(".approve-entry").addEventListener("click", async () => act("admin_approve_entry", { p_request_id: card.dataset.requestId, p_parent_member_id: null }));
  ...
  async function loadCommerceOrders() {
    ...
    updateCount("orders", openOrders.filter(order => ["pending_shipping_fee", "payment_submitted", "payment_approved"].includes(order.status)).length);
    ...
    commerceOrderList.innerHTML = openOrders.map(order => {
      ...
      const shippingDetails = order.shippedAt ? `<p class="withdrawal-history-note"><strong>Shipping:</strong> ${escapeHtml(order.courierName || "J&T")} ${order.trackingNumber ? `&middot; ${escapeHtml(order.trackingNumber)}` : ""} &middot; ${new Date(order.shippedAt).toLocaleString()}${order.shippingNotes ? ` &middot; ${escapeHtml(order.shippingNotes)}` : ""}</p>` : "";
      const upline = order.matrixUpline || {};
    ...
    commerceOrderList.querySelectorAll("[data-commerce-order-id]").forEach(card => {
      ...
      const shipForm = card.querySelector(".commerce-admin-ship-form");
      if (shipForm) {
        shipForm.addEventListener("submit", async event => {
          event.preventDefault();
          await act("admin_mark_commerce_order_shipped", {
            p_order_id: orderId,
            p_courier_name: card.querySelector(".commerce-admin-courier").value.trim(),
            p_tracking_number: card.querySelector(".commerce-admin-tracking").value.trim(),
            p_shipping_notes: card.querySelector(".commerce-admin-shipping-note").value.trim()
          });
        });
      }
    });
