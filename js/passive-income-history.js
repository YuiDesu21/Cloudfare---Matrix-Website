document.addEventListener("DOMContentLoaded", async () => {
  ...
  const entries = Array.isArray(summary && summary.rewardLedger) ? summary.rewardLedger : [];
  ...
  function render() {
    ...
    list.innerHTML = [...filtered].sort((a, b) => new Date(b.dueAt) - new Date(a.dueAt)).map(entry => {
      ...
      const source = entry.sourceType === "entry" ? "Entry" : `Exit ${entry.exit}`;
  ...
  function matchesSource(entry) { return sourceFilter.value === "all" || entry.sourceType === sourceFilter.value; }
